      ...
      <div className="w-full max-w-4xl mx-auto px-0.5 sm:px-4 h-14 md:h-15 flex items-center gap-1 md:gap-2">
        ...
        <div className="flex-1 min-w-0 relative" ref={feedMenuRef}>
          ...
          <button
            ...
            className={`w-full h-10 px-3 rounded-md border border-edge bg-surface text-sm text-fg items-center justify-between hover:bg-surface-hover transition-colors ${searchOpen ? "hidden" : "inline-flex md:hidden"}`}
            aria-haspopup="menu"
          ...
          <nav className={`hidden w-full items-center gap-1.5 ${searchOpen ? "" : "md:flex"}`}>
            {feeds.map((feed) => {
              const isActive = activeFeed === feed.type;
              const Icon = feed.icon;
              return (
